...
export async function POST(request: Request) {
  ...
  if (process.env.DISCORD_WEBHOOK_URL) {
    tasks.push(notifyDiscord(process.env.DISCORD_WEBHOOK_URL, inquiry))
  ...
  if (process.env.RESEND_API_KEY && process.env.CONTACT_FROM_EMAIL) {
    tasks.push(sendEmail(
      ...
      process.env.CONTACT_TO_EMAIL || 'hello@misslouisebakery.com',
      inquiry,
    ))
    channels.push('email')
  }
  ...
  results.forEach((result, index) => {
    if (result.status === 'rejected') console.error(`${channels[index]} delivery failed`, result.reason)
  })
